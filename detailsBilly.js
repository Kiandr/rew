...
(function () {
  ...
  ;
    ...
    var $tabset = $('#listing-details .tabset').on('click', 'a[data-target]', function () {
        ...
        if (typeof mapInstance === 'object' && typeof $map.REWMap('getTooltip') ==='object') {
            $map.REWMap('getTooltip').hide(true);
        }

        // Show map & directions
        if (target === 'map' && typeof $map === 'object') {
            //REWMOD Billy 09/26/2018 Prevent preload of Map
            // Show map container
            /*
        ...
        //REWMOD Billy 09/26/2018 Prevent preload of Streetview
        //if (target === 'streetview' && typeof streetview === 'object') {
        if (target === 'streetview') {
            ...
            var streetview = new REWMap.Streetview({
				...
				onFailure : function (data) {
					$('#map-streetview').html('Streetview unavailable for this property');
					$('#map-streetview').css('padding-top','3em');
					$('#map-streetview').css('text-align','center');
				}
			});
        }

        // Load bird's eye view on first click
        if (target === 'birdseye' && $birdseye.length > 0 && typeof birdseye !== 'object') {
 ...
 ;
    ...
    if (typeof IDX_LISTING === 'object') {
        ...
        // Load Google Streetview
        // REWMOD Billy 09/26/2018 No preload of streetview.
        //var $streetview = $('#map-streetview');
        var $streetview = $('#tab-streetview');
        /*
